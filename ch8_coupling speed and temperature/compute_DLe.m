function DLe = compute_DLe(JXYe, velocityE, rho, Cv, gaussPts, gaussWts)
...
for i = 1:length(gaussWts)
    for j = 1:length(gaussWts)
        ...
        %%%%%%% DLe单元方程子块计算
        DLe = DLe + gaussWts(i)*gaussWts(j)*rho*Cv*(fyV'*velocityE(:,1)*fyV*fy_x(1,:)...
            +fyV'*velocityE(:,2)* fyV*fy_x(2,:)) * det_Jacobi;
        %%%%%%% DLe单元方程子块计算
